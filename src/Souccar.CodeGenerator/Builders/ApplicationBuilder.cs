...
namespace Souccar.CodeGenerator
{
    public class ApplicationBuilder
    {
        ...
        public static void GenerateDtos(Type entityType, string folderPath)
        {
            ...
            GeneratePageRequestDtoFile(entityType, dtoPath);
        ...
        private static void GeneratePageRequestDtoFile(Type entityType, string dtoPath)
        {

            var fileName = $"Paged{entityType.Name}RequestDto.cs";
            var className = $"Paged{entityType.Name}RequestDto";
            var path = Path.Combine(dtoPath, fileName);

            if (!File.Exists(path))
            {
                var dtoBuilder = new DtoBuilder(entityType, "", false);
                var text = dtoBuilder.GeneratePagedRequest(className);
